...
class PatchTalkMcpFacade(
    private val commentService: PatchTalkCommentService,
) {
    ...
    fun createFileComment(path: String, body: String, author: String = "agent"): McpCommentThreadDto {
        return toDto(commentService.createFileThread(path, body, parseAuthor(author)))
    }

    fun createLineComment(path: String, line: Int, body: String, author: String = "agent"): McpCommentThreadDto {
        return toDto(commentService.createLineThread(path, line, body, parseAuthor(author)))
    }

    fun replyToComment(threadId: String, body: String, author: String = "agent"): McpCommentThreadDto {
    ...
    private fun parseAuthor(author: String): CommentAuthorType {
        return when (author.trim().uppercase()) {
            ...
            "AGENT", "CODEX" -> CommentAuthorType.AGENT
    ...
    private fun toDto(message: CommentMessage): McpCommentMessageDto {
        return McpCommentMessageDto(
            id = message.id,
            author = when (message.authorType) {
                CommentAuthorType.USER -> "USER"
                CommentAuthorType.AGENT, CommentAuthorType.CODEX -> "AGENT"
            },
            body = message.body,
