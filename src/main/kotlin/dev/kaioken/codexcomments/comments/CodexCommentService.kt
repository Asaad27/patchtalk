...
class CodexCommentService : PersistentStateComponent<CodexCommentProjectState> {
    private var state = CodexCommentProjectState()
    private val listeners = CopyOnWriteArrayList<CodexCommentListener>()

    override fun getState(): CodexCommentProjectState = state.deepCopy()

    override fun loadState(state: CodexCommentProjectState) {
        this.state = state.deepCopy()
        notifyChanged()
    }

    fun addListener(listener: CodexCommentListener) {
        listeners += listener
    }

    fun removeListener(listener: CodexCommentListener) {
        listeners -= listener
    }
