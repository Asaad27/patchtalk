# PatchTalk Plugin Design

## Goal

Build a JetBrains plugin that lets a user create local review-style comment threads in the IDE outside pull requests, then lets an MCP-compatible agent read, reply to, and resolve those threads.
...
That choice gives the plugin a durable project-scoped store without polluting source files. It also makes the state readable by PatchTalk through normal filesystem access if MCP is unavailable, while still letting the JetBrains plugin own the UI.
...
All tool outputs return structured serializable DTOs so PatchTalk can parse them reliably.
...
- Agent actions on comments only affect plugin-owned thread state unless the user separately asks the agent to edit code.
